        ...
        {ITEMS.map((item, i) => {
          ...
          return (
            ...
            >
              <Link
                to={item.href}
                className={`group relative flex h-full min-h-[340px] flex-col overflow-hidden rounded-[26px] p-5 shadow-[0_18px_50px_-25px_rgba(0,0,0,0.45)] transition-shadow hover:shadow-[0_28px_70px_-25px_rgba(0,0,0,0.55)] ${t.card}`}
              >
                <span
                ...
                <h3
                  className={`mt-3 whitespace-pre-line font-grotesk text-[26px] font-bold leading-[1.05] tracking-tight sm:text-[28px] ${t.title}`}
                >
                  {item.title}
                </h3>
                ...
                <div className="pointer-events-none relative mt-auto h-[160px] w-full">
                  <img
                    src={item.art}
                    alt=""
                    loading="lazy"
                    aria-hidden="true"
                    className="absolute -bottom-2 right-0 h-[200px] w-auto max-w-[120%] object-contain transition-transform duration-700 group-hover:scale-[1.04]"
                  />
                </div>
              </Link>
            </motion.div>
