  ...
  return (
    <section className="section-x">
      <div className="relative overflow-hidden rounded-[22px] border border-black/5 bg-[#f6f5f0] px-4 pb-5 pt-6 shadow-[0_30px_80px_-40px_rgba(0,0,0,0.25)] sm:rounded-[32px] sm:px-8 sm:pb-10 sm:pt-12">
        ...
        {/* Cards */}
        <div className="relative mt-6 flex snap-x snap-mandatory gap-2 overflow-x-auto pb-2 hide-scrollbar sm:mt-10 sm:grid sm:grid-cols-3 sm:gap-4 sm:overflow-visible">
          {ITEMS.map((item, i) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -4 }}
                className="h-full w-[58vw] max-w-[200px] flex-shrink-0 snap-center sm:w-auto sm:max-w-none"
              >
                <Link
                  to={item.href}
                  className={`group relative flex h-full min-h-[170px] flex-col overflow-hidden rounded-[16px] p-2.5 shadow-[0_18px_50px_-25px_rgba(0,0,0,0.45)] transition-shadow hover:shadow-[0_28px_70px_-25px_rgba(0,0,0,0.55)] sm:min-h-[320px] sm:rounded-[26px] sm:p-5 ${t.card}`}
                >
                  <span
                    className={`inline-flex w-fit items-center gap-1 rounded-full px-1.5 py-0.5 text-[8px] font-medium sm:px-2.5 sm:py-1 sm:text-[11px] ${t.chip}`}
                  >
                  ...
                  <h3
                    className={`mt-1.5 whitespace-pre-line font-grotesk text-[13px] font-bold leading-[1.05] tracking-tight sm:mt-3 sm:text-[26px] ${t.title}`}
                  >
                    {item.title}
                  </h3>
                  <p className={`mt-1 line-clamp-2 text-[10px] leading-snug sm:mt-1.5 sm:line-clamp-none sm:text-[13px] ${t.sub}`}>
                    {item.subtitle}
                  </p>

                  <div className="pointer-events-none relative mt-auto h-[70px] w-full sm:h-[150px]">
                    <img
                      src={item.art}
                      alt=""
                      loading="lazy"
                      aria-hidden="true"
                      className="absolute -bottom-1 right-0 h-[95px] w-auto max-w-[125%] object-contain transition-transform duration-700 group-hover:scale-[1.04] sm:-bottom-2 sm:h-[190px]"
                    />
                  </div>
                </Link>
              </motion.div>
